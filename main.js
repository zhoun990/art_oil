...
const App = () => {
	...
	const calcDefBtwPs = (x1, y1, x2, y2, radius1, radius2) => {
		return Math.sqrt(
			Math.pow(x2 + radius2 - x1 + radius1, 2) +
				Math.pow(y2 + radius2 - y1 + radius1, 2)
		);
	};
				...
				for (let j = 0; j < array.length; j++) {
					if (i !== j) {
						...
						if (
							...
							y1 + ni > y2 - nj &&
							y1 - ni < y2 + nj &&
							calcDefBtwPs(x1, y1, x2, y2, ni, nj) < ni + nj
						) {
							if (!deleteList.includes(i) && !deleteList.includes(j)) {
								if (
									!array[i].rest &&
									!array[j].rest &&
									hitAndDelete &&
									...
									if (!array[i].splitCoolTime && !array[j].splitCoolTime) {
										...
										(array[i].color == "white" || array[i].color == "black")
									) {
										...
										array[i].color =
											array[i].color === "black" ? "black" : newColor;
										array[j].color =
											array[j].color === "black" ? "black" : newColor;
									} else {
										array[i].color = array[j].color;
	...
	const start = () => {
		...
		for (
			let i = 0;
			document.body.clientWidth > 700 ? i < gen : i < Math.floor(gen / 2);
			i++
		) {
	...
	useEffect(() => {
		if (points.length && isEnable && !timeoutId) {
			setTimeoutId(
				setInterval(() => update(refPoints.current.concat()), interval)
			);
